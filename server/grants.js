...
export function openGrants(db) {
  ...
  const onCard = one('select * from grants where card = ? order by created')
  const forPerson = one(
  ...
  return {
    ...
    give(card, { person = null, scope = 'read', by = null, expires = null } = {}) {
      ...
      if (person) {
        ...
        return { id, kind: 'person', subject: person, scope, expires }
      }
    ...
    on(card) {
      return onCard.all(card).map(({ subject, ...rest }) => rest)
    },
