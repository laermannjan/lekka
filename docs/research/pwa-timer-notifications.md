...
- **"Backgrounded but open" vs "fully closed/swiped away" vs "phone
  ...
  - *Phone locked, app not running*: works for **push-delivered**
  ...
  - *App backgrounded (user switched to another app) but not swiped away*:
  ...
  - *App fully closed/terminated*: only push-delivered notifications work;
...
- **Chrome timer throttling has two escalating tiers**, documented
  ...
  - *Standard throttling*: applies once a page is hidden; the browser
    checks/executes chained timers once per **second**.
  - *Intensive throttling* (new in Chrome 88, Jan 2021): once a page has
