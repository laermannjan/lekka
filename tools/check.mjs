...
addEventListener('load', async () => {
  ...
  // Opened at a link with a grant token in the path, the way every older link is written.
  check('a token in the path is read, and rewritten into the fragment',
...
try {
  ...
  lines.push(...await said(chrome, port, `/r/${made.id}/atokenfromalink`, '1280,900'))
