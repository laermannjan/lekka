<script lang="ts">
	...
	// PROTOTYPE (throwaway) - three UI variants gated behind `?variant=`.
	// Remove these four imports, the `variant` derived below, and the
	// `{#if variant}` wrapper around the markup once one has won.
	import { page } from '$app/state';
	import PrototypeSwitcher from '$lib/components/PrototypeSwitcher.svelte';
	import PrototypeVariantA from './PrototypeVariantA.svelte';
	import PrototypeVariantB from './PrototypeVariantB.svelte';
	import PrototypeVariantC from './PrototypeVariantC.svelte';
	import {
		cancelTimerPush,
		isPushSupported,
		isSubscribed,
	...
	// PROTOTYPE - null means "render the real page".
...
<!-- PROTOTYPE wrapper - see the imports above. -->
<PrototypeSwitcher
	variants={['A', 'B', 'C']}
	names={{ A: 'Kitchen Counter', B: 'Workbench', C: 'Cook Mode' }}
/>

{#if variant === 'A'}
	<PrototypeVariantA {data} {timers} />
{:else if variant === 'B'}
	<PrototypeVariantB {data} {timers} />
{:else if variant === 'C'}
	<PrototypeVariantC {data} {timers} />
{:else}
	...
	<details>
		...
		<p>
			Every stored quantity and duration is written "as usual" at this count - viewing at a
			different count above recomputes from this baseline.
		</p>
											...
											{#if usage.alternativeIngredient}
												{#if clearingAlternativeUsageIds.has(usage.id)}
													Suggested swap: {usage.alternativeIngredient
														...
														({usage.alternativeIngredient.descriptors}){/if}.
												{:else}
													The declared alternative is also avoided by a selected diner, so no swap
													is suggested.
												{/if}
											{:else}
						...
						<form method="POST" action="?/overrideStep">
							...
							<fieldset>
								...
								{#if step.durationScalingFormula}
									<p>
										This duration has a scaling rule. Clearing the duration removes the rule with
										it, and changing the unit drops a "vs. another usage" rule, whose per-unit
										amount is written in the unit below.
									</p>
								{/if}
								<label>
									Kind
	...
	<p>
		Logging a cook records what happened, when, and by whom - it never edits this recipe itself.
	</p>
	...
	{#if data.cooks.length > 0}
		<ol>
			{#each data.cooks as cook (cook.id)}
				...
				{@const cookedComposition = data.recipe.compositions.find(
					(c) => c.id === cook.compositionId
				)}
				<li>
					<p>
